...
void ofxOculusDK2::fullscreenOnRift() {
    ...
    int displayHeight= CGDisplayPixelsHigh ( displays[0] );
    int displayWidth= CGDisplayPixelsWide ( displays[0] );
    CGRect displayBounds= CGDisplayBounds ( displays[0] );
...
void ofxOculusDK2::renderGui(ovrEyeType eye){
        glPushAttrib(GL_ALL_ATTRIB_BITS);
        glDisable(GL_LIGHTING);
        ofDisableDepthTest();
//        ofSetColor(255, 0, 0);
...
void ofxOculusDK2::draw(){
	...
	distortionShader.setUniformTexture("Texture", renderTarget.getTexture(), 1);
